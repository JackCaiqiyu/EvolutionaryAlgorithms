...
public class CEC14Benchmark extends AllBenchmarks {
    ...
    public double f(double[] x) {
        ...
        if (ini_flag) /* initiailization*/
        {
			...
			/*Load Matrix M*****************************************************/
            File fpt = new File("input_data/M_"+FUN+"_D"+nx+".txt");//* Load M data *
            Scanner input = null;
            try {
            ...
            if (FUN<23)
            {
                fpt=new File("input_data/shift_data_"+FUN+".txt");
                try {
                    input = new Scanner(fpt);
                } catch (FileNotFoundException e) {
                    System.out.println("\n Error: Cannot open input file for reading ");
            ...
            if (FUN>=17&&FUN<=22)
            {
                fpt = new File("input_data/shuffle_data_"+FUN+"_D"+nx+".txt");
