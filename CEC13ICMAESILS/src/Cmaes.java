       ...
       // cma.writeToDefaultFilesHeaders(0); // 0 == overwrites old files
        ...
        while ((cma.getCountEval() < cma.options.stopMaxFunEvals) && (bestValue - bias.getBias(F) > cma.options.stopFitness)){
            ...
            for(int i = 0; i < pop.length; ++i) {    // for each candidate solution i
                ...
                while (!inbound(Bounds.getLowerBound(Configuration.F), Bounds.getUpperBound(Configuration.F), pop[i], Configuration.DIM))     //   not located on (or very close to) the domain boundary,
